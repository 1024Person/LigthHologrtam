function CGH = LM3F(im,r,c)
% 罗曼三型编码

%  但是这种编码方式还不是最好的编码方式，所以需要以后去更改一下，
%  用博奇编码应该会更好一些
...
% imshow(U,[]);
[r,c] = size(U);
% ef1 =0.9;         % 这里的ef1越大，再现的图像就越清晰，这里可以深究一下，
% 这里的思考方向可以是在时域上的乘上一个相位然后频域上对应着的操作，我记得好像是频移！！！
UF = fftshift(fft2(U));
s =6;      % 设置编码单元的大小
w =s;      % 设置通光孔径的宽度,通光孔径设置成s，最合适！！，
% 只是测试了一下整数，因为W控制着透过率，如果W越高，那么透过率就越大，透过的能量就越多，显示的就越清楚
...
ef2 =2.0;  % 滤波操作，这里对于每一个图片的阈值设置不一样，我读进来一个200x200规格的图片，对这个图片的滤波操作最好在4
th = maxAm / ef2;
Am(Am > th) = th;
...
% RI=RU.*conj(RU);
% figure(3);imshow(RI,[0,max(RI(:))/1000]);
% colormap(pink)
end