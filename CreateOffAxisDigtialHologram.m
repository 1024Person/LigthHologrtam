...
N1 = min([M0,N0]);
N = 1000;   % 模拟形成的全息图的取样数
X1 = imresize(X0, N/4/N1);  % X1是初始物光场尺寸的的放大，
%   因为是菲涅耳衍射，衍射前后，光场的尺寸是不会发生变化的，所以要先保持前后尺寸相同，
%   经过计算得到，最后的复现图的尺寸是物光场尺寸的4倍，所以这里就要先进行放大，
%   之后和离轴参考光进行相干叠加，然后得到全息图，这时候得到的全息图就是4倍的物光场尺寸的四倍
...
f2 = f.*Fresnel;
f2 = fftshift(f2);      %  制作全息图的时候，还是相位的关系没有搞好阿，这里还要先移动一下
Uf = fft2(f2,N,N);
Uf = fftshift(Uf);
% 观察平面
